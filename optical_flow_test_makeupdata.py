...
#1.Load data
all_images = skimage.io.imread('makeup_data123.tif',as_gray=True)

#all_images = skimage.io.imread('new_made_up_data.tif')
#all_images = tifffile.imread('new_made_up_data.tif')
...
#skimage.io.imsave('all_blurred_makeupdata.tif', blurred_images)
I = all_images[0,:,:]
...
all_local_error_ij=np.zeros((number_of_frames-1,Nb,Nb))
...
#                 skimage.io.imsave('Vy_velocity_only.tif', v_y[box_index_x,box_index_y])
                ...
                #some error calculation debugging code
                if frame_index == 1:
                    true_v_x=1/0.019550342130987292
                    true_v_y=2/0.019550342130987292
                    this_true_gamma=0
                else: 
                    true_v_x=0
                    true_v_y=0 
                    this_true_gamma=0.5
                true_error_this_box =0
                for x_index in range(box_index_x*box_size,(box_index_x+1)*box_size-1):
                    for y_index in range(box_index_y*box_size,(box_index_y+1)*box_size-1):
                        this_pixel_error = (delta_I[x_index,y_index]+
                                            delta_t*true_v_x*dIdx[x_index,y_index]+
                                            delta_t*true_v_y*dIdy[x_index,y_index]-delta_t*this_true_gamma)
                        this_square_error = this_pixel_error*this_pixel_error
                        true_error_this_box += this_square_error
                #true_local_error_ij[box_index_x,box_index_y]= error_this_box
                print('the true error ought to be')
                print(true_error_this_box)
                
                actual_error_this_box =0
                for x_index in range(box_index_x*box_size,(box_index_x+1)*box_size-1):
                    for y_index in range(box_index_y*box_size,(box_index_y+1)*box_size-1):
                        this_pixel_error = (delta_I[x_index,y_index]+
                                            delta_t*Vx*dIdx[x_index,y_index]+
                                            delta_t*Vy*dIdy[x_index,y_index]-delta_t*this_gamma)
                        this_square_error = this_pixel_error*this_pixel_error
                        actual_error_this_box += this_square_error
                #true_local_error_ij[box_index_x,box_index_y]= error_this_box
                print('the actual error is')
                print(actual_error_this_box)
                

all_v_x[~np.isnan(all_v_x)]    
all_v_y[~np.isnan(all_v_y)] 
all_gamma[~np.isnan(all_gamma)]    
 
#e=10, all_gamma[1,0,0]=-0.4155155668071664, gamma should be 0.5, it's minus because of darker
...
for frame_index in range(1,blurred_images.shape[0]):
    ...
    for box_index_x in range(Nb):
        for box_index_y in range(Nb):
                this_dIdx_box = np.mean(dIdx[box_index_x*box_size:box_index_x*box_size+box_size,box_index_y*box_size:box_index_y*box_size+box_size])
                dIdx_box[box_index_x,box_index_y] = this_dIdx_box
                this_dIdy_box = np.mean(dIdy[box_index_x*box_size:box_index_x*box_size+box_size,box_index_y*box_size:box_index_y*box_size+box_size])
...
#Check local error_ij=Chi^2 parentheses=delta mu_ij+delta t*Vx*mu_x+delta t*Vy*mu_y=delta t*gamma
all_local_error_ij=np.zeros((number_of_frames-1,Nb,Nb))
for frame_index in range(1,blurred_images.shape[0]):
    local_error_ij = all_local_error_ij[frame_index-1,:,:]
    for box_index_x in range(Nb):
        for box_index_y in range(Nb):           
            this_local_error_ij = all_difference_to_previous_frame_box[frame_index-1,box_index_x,box_index_y]+delta_t*all_v_x[frame_index-1,box_index_x,box_index_y]*all_dIdx_box[frame_index-1,box_index_x,box_index_y]+delta_t*all_v_y[frame_index-1,box_index_x,box_index_y]*all_dIdy_box[frame_index-1,box_index_x,box_index_y]-delta_t*all_gamma[frame_index-1,box_index_x,box_index_y]
            local_error_ij[box_index_x,box_index_y]= this_local_error_ij
            print(this_local_error_ij)
  #relative error 
all_relative_local_error_ij=np.zeros((number_of_frames-1,Nb,Nb))
for frame_index in range(1,blurred_images.shape[0]):
    relative_local_error_ij = all_relative_local_error_ij[frame_index-1,:,:]
    for box_index_x in range(Nb):
        for box_index_y in range(Nb):           
            this_relative_local_error_ij = (all_difference_to_previous_frame_box[frame_index-1,box_index_x,box_index_y]+delta_t*all_v_x[frame_index-1,box_index_x,box_index_y]*all_dIdx_box[frame_index-1,box_index_x,box_index_y]+delta_t*all_v_y[frame_index-1,box_index_x,box_index_y]*all_dIdy_box[frame_index-1,box_index_x,box_index_y]-delta_t*all_gamma[frame_index-1,box_index_x,box_index_y])/all_difference_to_previous_frame_box[frame_index-1,box_index_x,box_index_y]
            relative_local_error_ij[box_index_x,box_index_y]= this_relative_local_error_ij
            print(this_relative_local_error_ij)

         
all_true_local_error_ij=np.zeros((number_of_frames-1,Nb,Nb))
for frame_index in range(1,blurred_images.shape[0]):
    if frame_index == 1:
        X=1/0.019550342130987292
        Y=2/0.019550342130987292
        G=0
    else: 
        X=0
        Y=0 
        G=0.5
    true_local_error_ij = all_true_local_error_ij[frame_index-1,:,:]
    for box_index_x in range(Nb):
        for box_index_y in range(Nb):           
            this_true_local_error_ij = all_difference_to_previous_frame_box[frame_index-1,box_index_x,box_index_y]+delta_t*X*all_dIdx_box[frame_index-1,box_index_x,box_index_y]+delta_t*Y*all_dIdy_box[frame_index-1,box_index_x,box_index_y]-delta_t*G
            true_local_error_ij[box_index_x,box_index_y]= this_true_local_error_ij
            print(this_true_local_error_ij)
            
  
           
all_new_true_local_error_ij=np.zeros((number_of_frames-1,Nb,Nb))
for frame_index in range(1,blurred_images.shape[0]):
    if frame_index == 1:
        v_x=1/0.019550342130987292
        v_y=2/0.019550342130987292
        this_gamma=0
    else: 
        v_x=0
        v_y=0 
        this_gamma=0.5
    true_local_error_ij = all_new_true_local_error_ij[frame_index-1,:,:]
    for box_index_x in range(Nb):
        for box_index_y in range(Nb):  
            error_this_box =0
            for x_index in range(box_index_x*box_size,(box_index_x+1*box_size)):
                for y_index in range(box_index_x*box_size,(box_index_x+1*box_size)):
                    this_pixel_error = difference_to_previous_frame[frame_index-1,x_index,y_index]+delta_t*v_x*dIdx_box[frame_index-1,x_index,y_index]+delta_t*v_y*dIdy[frame_index-1,x_index,y_index]-delta_t*this_gamma
                    this_square_error = this_pixel_error*this_pixel_error
                    error_this_box += this_square_error
            true_local_error_ij[box_index_x,box_index_y]= error_this_box
            print(this_true_local_error_ij)
